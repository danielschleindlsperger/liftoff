...
(defn parse-openapi-schema
  ...
  {:liftoff/openapi        (jsonista/read-value raw-schema)
   :liftoff/handler-config {"updatePet"  {:validations {:parameters {:path  nil
                                                                     :query nil
                                                                     :body  {"application/json"                  {"schema" {"$ref" "#/components/schemas/Pet"}}
...
(defn wrap-operation-id
  ...
  (fn [req]
    (handler (assoc req :liftoff/operation-id operation-id))))
...
(defn wrap-validate-openapi
  ...
  (fn [req]
    ...
    (let [operation-id (get-in req [:liftoff/operation-id])
          param-validations (get-in liftoff-schema [:liftoff/handler-config operation-id :validations :parameters])]

      (prn operation-id)
      (prn param-validations)

      (when (:path param-validations)
        (def valid? (jinx.validate/validate
...
(defn ->ring
  ...
  (let [routes (map (fn
                      [[path methods]]
                      (let [path-config (into {} (map (fn
                                                        [[method method-config]]
                                                        (let [operation-id (get method-config "operationId")
                                                              handler (get-in liftoff-schema [:liftoff/handlers (keyword operation-id)])]
                                                          ;; TODO: validate that we have a handler defined for the given operationId (and also that the operationId is defined in the schema)
                                                          [(keyword method) {:operation-id operation-id
                                                                             :handler      handler
                                                                             :middleware   [[wrap-operation-id operation-id]
                                                                                            [wrap-validate-openapi liftoff-schema]]}]))
                                                      methods))]
                        [path path-config]))
                    (get-in liftoff-schema [:liftoff/openapi "paths"]))]
    ;; Routes with parameters can have conflicts with explicit routes, such as /pets/findByTags and /pets/{petId}.
    ;; For a simpler implementation we just ignore the conflicts for now instead of building up a nested route tree which might solve the issue.
    (ring/ring-handler (ring/router routes {:conflicts nil}))))

(comment
  (def raw-schema (slurp (io/resource "examples/pet-store.json")))

  (def handlers {:getPetById        (fn [req] {:status 200 :body "getPetById"})
